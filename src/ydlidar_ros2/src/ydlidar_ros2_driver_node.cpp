...
int main(int argc, char *argv[]) {
  ...
  node->get_parameter<std::string>("ignore_array", str_optvalue);
  ...
  node->get_parameter<bool>("fixed_resolution", b_optvalue);
  laser.setlidaropt(LidarPropFixedResolution, &b_optvalue, sizeof(bool));
  /// rotate 180
  b_optvalue = true;
  node->declare_parameter<bool>("reversion");
  node->get_parameter<bool>("reversion", b_optvalue);
  laser.setlidaropt(LidarPropReversion, &b_optvalue, sizeof(bool));
  /// Counterclockwise
  b_optvalue = true;
  node->declare_parameter<bool>("inverted");
  node->get_parameter<bool>("inverted", b_optvalue);
  laser.setlidaropt(LidarPropInverted, &b_optvalue, sizeof(bool));
  b_optvalue = true;
  node->declare_parameter<bool>("auto_reconnect");
  node->get_parameter<bool>("auto_reconnect", b_optvalue);
  laser.setlidaropt(LidarPropAutoReconnect, &b_optvalue, sizeof(bool));
  /// one-way communication
  b_optvalue = true;
  node->declare_parameter<bool>("isSingleChannel");
  node->get_parameter<bool>("isSingleChannel", b_optvalue);
  laser.setlidaropt(LidarPropSingleChannel, &b_optvalue, sizeof(bool));
  /// intensity
  b_optvalue = false;
  node->declare_parameter<bool>("intensity");
  node->get_parameter<bool>("intensity", b_optvalue);
  laser.setlidaropt(LidarPropIntenstiy, &b_optvalue, sizeof(bool));
  /// Motor DTR
  b_optvalue = false;
  node->declare_parameter<bool>("support_motor_dtr");
  node->get_parameter<bool>("support_motor_dtr", b_optvalue);
  ...
  node->get_parameter<float>("angle_max", f_optvalue);
  laser.setlidaropt(LidarPropMaxAngle, &f_optvalue, sizeof(float));
  f_optvalue = -180.0f;
  node->declare_parameter<float>("angle_min");
  node->get_parameter<float>("angle_min", f_optvalue);
  laser.setlidaropt(LidarPropMinAngle, &f_optvalue, sizeof(float));
  /// unit: m
  f_optvalue = 8.f;
  node->declare_parameter<float>("range_max");
  node->get_parameter<float>("range_max", f_optvalue);
  laser.setlidaropt(LidarPropMaxRange, &f_optvalue, sizeof(float));
  f_optvalue = 0.1f;
  node->declare_parameter<float>("range_min");
  node->get_parameter<float>("range_min", f_optvalue);
  laser.setlidaropt(LidarPropMinRange, &f_optvalue, sizeof(float));
  /// unit: Hz
  f_optvalue = 8.f;
  node->declare_parameter<float>("frequency");
  node->get_parameter<float>("frequency", f_optvalue);
  laser.setlidaropt(LidarPropScanFrequency, &f_optvalue, sizeof(float));

  bool invalid_range_is_inf = false;
  node->declare_parameter<bool>("invalid_range_is_inf");
  node->get_parameter<bool>("invalid_range_is_inf", invalid_range_is_inf);
